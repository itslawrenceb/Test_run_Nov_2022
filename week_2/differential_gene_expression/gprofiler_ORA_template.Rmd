...
all_genes_1df <- read.csv("CD8_vs_CD4_DES_results_lfcshrinkage.csv")
...
all_genes_tested_1df <- all_genes_1df %>%
                          dplyr::filter(padj !="NA")

dim(all_genes_tested_1df)

  
# Subset 2 - All sig upregulated genes (padj < 0.05. & log2FoldChange > 1)
upreg_genes_df <- all_genes_tested_1df [(all_genes_tested_1df$padj < 0.05 & all_genes_tested_1df$log2FoldChange > 1), ]
dim(upreg_genes_df)

# Subset 3 - All sig downregulated genes (padj < 0.05. & log2FoldChange < 1)
downreg_genes_df <- all_genes_tested_1df [(all_genes_tested_1df$padj < 0.05 & all_genes_tested_1df$log2FoldChange < -1), ]
...
all_gene_ids <- all_genes_1df$ensembl_gene_ids
...
gost_result_obj <- gost(query = upreg_gene_ids, organism = "mmusculus",
exclude_iea = T, correction_method = "g_SCS", custom_bg = all_gene_ids)

# what are our outputs  
summary(gost_result_obj)
gost_result_obj$result
...
gostplot(gost_result_obj)
```

```{r}
# what does as_short_link = FALSE do in gost() function? 
gost_result_obj <- gost(query = upreg_gene_ids, organism = "mmusculus",
exclude_iea = T, correction_method = "g_SCS", custom_bg = all_gene_ids, as_short_link = T)
