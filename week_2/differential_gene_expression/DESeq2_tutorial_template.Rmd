...
vst_transformed_data <- vst(dds_filtered)
...
mypca_1 <- plotPCA(rlog_transformed_dds_subset, intgroup = "cell_type") +
...
mypca_2 <- plotPCA(rlog_transformed_dds_subset, intgroup = "genotype") +
  theme(aspect.ratio = 1)
mypca_2
...
mypca_3 <- plotPCA(rlog_transformed_dds_subset, intgroup = "cell_type", ntop = nrow(rlog_transformed_dds_subset)) +
  theme(aspect.ratio = 1)
mypca_3
...
dds_normalized_counts <- counts(dds_filtered_updated, normalized = T)

# check the class of the output
head(dds_normalized_counts)
dds_normalized_counts
```
...
plotCounts(dds_final, gene = "ENSMUSG00000041779", returnData = T, intgroup = "cell_type")
```
...
results_CD4_CD8 <- results(dds_final, alpha = 0.5, contrast = c("cell_type", "CD4", "CD8"))
results_CD8_CD4 <- results(dds_final, alpha = 0.5, contrast = c("cell_type", "CD8", "CD4"))
...
Shruncken_LFC_dds_results_obj <- lfcShrink(dds_final, coef = "cell_type_CD8_vs_CD4")
Shruncken_LFC_dds_results_obj
```
...
shrunkenLFC_dds_results_obj_df <- as.data.frame(Shruncken_LFC_dds_results_obj)

# Use the org.Mm.eg.db package to get gene symbols for the ensembl gene ids (hint:you did this on bioconductor day) 
genesymbole <- mapIds(org.Mm.eg.db,
keys = rownames(shrunkenLFC_dds_results_obj_df), column = "SYMBOL",
keytype = "ENSEMBL")
...
shrunkenLFC_dds_results_obj_df$Gene_Symbol <- genesymbole
shrunkenLFC_dds_results_obj_df
```
...
shrunkenLFC_dds_results_obj_df <- shrunkenLFC_dds_results_obj_df %>% 
						rownames_to_column("ensembl_gene_ids")

# Save results to file 
write_csv(shrunkenLFC_dds_results_obj_df, "CD8_vs_CD4_DE_results_lfcshrinkage_allgenes.csv")
```
