...
## another way
```{r}
#iris_features <- mutate(iris, Species = Null)
#head(iris_features)
```
...
set.seed(1)
pca_matrix <- as.matrix(pca_iris$x)

umap_iris <- umap(pca_matrix)
...
ggplot(umap_iris_dataframe, aes(V1, V2, color = Species)) +
  geom_point()

```

```{r}
umap_iris_species <- ggplot(umap_iris_dataframe, aes(V1, V2, color = Species)) +
  geom_point()
...
```{r}
cluster_cord <- kmeans_iris$centers
```
...
```{r}
iris_means_centers <- as.data.frame(cluster_cord )
...
geom_point(aes(color = Kmeans))  +
geom_point(data=iris_means_centers, shape = "*", size = 15, color = "black")
```
