...
```{r}
iris_raw %>%
  mutate("sepal_area" = sepal_width * sepal_length)
...
> Answer:We do  ot know the specie flower with each sepal_length,sepal_width,petal_length, and petal_width
...
exprs_assay <- read_xlsx("EH2011.xlsx", sheet = "exprs")
...
ggplot(exprs_assay_long, aes(x=value)) +
  geom_histogram(fill = "purple", bins = 30, aes(color = gene)) +
  facet_wrap(~gene)
...
```{r}
expression_diff <- exprs_sampleinfo %>%
group_by(sex, gene) %>%
...
expression_plot <- ggplot(table_for_ggplot, aes(x=grade, y=value, color = T)) +
geom_point()
facet_wrap(~sex)
...
```{r}
ggplot(exprs_sampleinfo, aes(unique_patient_ID, gene, fill = value)) +
  geom_tile() +
...
```{r}
ggplot(exprs_sampleinfo, aes(interaction(unique_patient_ID, sex), gene, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c() +
  theme(
    axis.text.x = element_blank(), axis.ticks.x = element_blank(),
