...
> Answer: The function returns a 'dgCMatrix', this is a type of space
> 
...
project = "pbmc5k", min.cells = 3, min.features = 200
...
seurat_object@assays$RNA[1:6, 1:6]
```
...
FetchData( seurat_object, object =   )
...
lyz <- FetchData(object = seurat_object, vars = c("nCounts_RNA", "nFeature_RNA", "LYZ"))
...
```{r}
violin_plot <- VlnPlot( object = seurat_object,
...
```{r}
seurat_after_qc <- subset(x = seurat_object, subset = nCount_RNA >4500 &
percent_mt < 15 & nFeature_RNA > 1500)
...
```{r}
ggplot_lyz_raw <- ggplot( FetchData(seurat_object, vars = c("LYZ"), slot = "counts"), aes(LYZ) ) +
    geom_histogram(  bins = 100 ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
ggplot_lyz_raw

ggplot_lyz_normalised <- ggplot( FetchData(seurat_after_qc, vars = c("LYZ"), slot = "data"), aes(LYZ) ) +
    geom_histogram(  bins = 100 ) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
